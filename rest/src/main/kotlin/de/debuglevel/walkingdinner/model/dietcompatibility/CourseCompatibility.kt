...
object CourseCompatibility : Compatibility {
    ...
    private fun isCapabilityCompatible(course: String, capabilities: List<Capability>, otherTeam: Team): Boolean {
        when (otherTeam.diet) {
            ...
            Diet.Vegetarisch -> {
                when (course) {
                    Courses.course1name -> return capabilities.contains(Capability.VeganVorspeise) || capabilities.contains(
                        Capability.VegetarischVorspeise
                    )
                    Courses.course2name -> return capabilities.contains(Capability.VeganHauptgericht) || capabilities.contains(
                        Capability.VegetarischHauptgericht
                    )
                    Courses.course3name -> return capabilities.contains(Capability.VeganDessert) || capabilities.contains(
                        Capability.VegetarischDessert
                    )
                }
            }
            Diet.Omnivore -> {
                when (course) {
                    Courses.course1name -> return capabilities.contains(Capability.VeganVorspeise) || capabilities.contains(
                        Capability.VegetarischVorspeise
                    ) || capabilities.contains(Capability.OmnivorVorspeise)
                    Courses.course2name -> return capabilities.contains(Capability.VeganHauptgericht) || capabilities.contains(
                        Capability.VegetarischHauptgericht
                    ) || capabilities.contains(Capability.OmnivorHauptgericht)
                    Courses.course3name -> return capabilities.contains(Capability.VeganDessert) || capabilities.contains(
                        Capability.VegetarischDessert
                    ) || capabilities.contains(Capability.OmnivorDessert)
                }
            }
        }
