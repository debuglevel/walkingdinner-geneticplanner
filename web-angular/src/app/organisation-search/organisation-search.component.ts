...
export class OrganisationSearchComponent implements OnInit {
  organisationes$: Observable<Organisation[]>;
  ...
  ngOnInit(): void {
    this.organisationes$ = this.searchTerms.pipe(
      ...
      switchMap((term: string) => this.organisationService.searchOrganisations(term)),
    );
  }
}